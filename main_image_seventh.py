...
from engine_finetune_second import train_one_epoch, evaluate
...
def main(args):
    ...
    dataset_train = SarEODataset(root_dir=args.data_path, transform=transform)
...
#  
    ...
    for _, p in model.patch_embed.named_parameters():
        p.requires_grad = True
    for _, p in model.patch_embed_dist.named_parameters():
        p.requires_grad = True
    ...
    if not os.path.exists("adjustment.pt"):
        adjustment_path = "adjustment.pt"
        adjustment = compute_adjustment(data_loader_train, tro=1.0)
        save_adjustment(adjustment, adjustment_path)

    adjustments = load_adjustment("adjustment.pt", device)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats, lmbda= train_one_epoch(
