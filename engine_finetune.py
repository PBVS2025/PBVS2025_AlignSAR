...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module, data_loader: Iterable, optimizer: torch.optim.Optimizer,
    ...
    for data_iter_step, batch in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.cuda.amp.autocast():
            if eo_input:
                outputs, outputs_dist, x_feat, x_dist_feat = model(samples, eo_input=True)
                                
                ce_loss = criterion(torch.cat([outputs+adjustments, outputs_dist+adjustments]), targets)
                ...
                outputs = model(samples, eo_input=False)
